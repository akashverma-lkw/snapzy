...
const Post = ({ post }) => {
	...
	const [undoClicked, setUndoClicked] = useState(false); // NEW: Track Undo click
	const undoTimeoutIdRef = useRef(null);
	...
	const handleDeletePost = () => {
		...
		toast(
			(t) => (
				<span>
					<span className="flex items-center gap-2">Post deleted <IoSadOutline /></span>
					<span className="flex items-center gap-2">Want to get back post ?<IoHappyOutline />
						<button
							onClick={() => {
								undoDelete(t.id);
							}}
							className="ml-2 px-2 py-1 bg-green-500 hover:bg-green-600 text-white rounded"
						>
		...
		// Set timeout and store its ID in ref
	...
	const undoDelete = (toastId) => {
		...
		// Add the post back into the cache
		setDeletedPost(null);

		// Close the toast and show success
